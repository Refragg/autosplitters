...
state("sonic")
{
	...
	byte missionClear : 0x03872964;
	byte chaos6Clear : 0x0371892C;
	byte kccCheck : 0x03852C04;
...
split
{
	...
	{				 
		if (current.level == 33 && current.act == 0) //the condition is split like this, because too many "&&" make the autosplitter not working for some reason... 
		{
			if (settings["deathWV"] && current.adventureData != old.adventureData && current.currCharacter == 6) {return true;}
		}
		
		if (current.level == 33 && current.act == 1) //the condition is split like this, because too many "&&" make the autosplitter not working for some reason... 
		{
	...
	{
		if (settings["EnterCave"] && current.level == 33 && current.act == 1 && old.act == 0) {return true;}
